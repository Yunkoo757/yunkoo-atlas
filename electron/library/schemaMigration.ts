...
function readMigrationMarker(paths: LibraryPaths): SchemaMigrationMarker | null {
  ...
  if (
    ...
    value === null ||
    marker.version !== 1 ||
    ![8, 9, 10, 11, 12, 13].includes(Number(marker.fromVersion)) ||
    ![9, 10, 11, 12, 13, SCHEMA_VERSION].includes(Number(marker.toVersion)) ||
...
function copyRecoveryPair(
  ...
  if (![8, 9, 10, 11, 12, 13].includes(manifest.schemaVersion) || manifest.schemaVersion >= SCHEMA_VERSION) {
  ...
  const marker: SchemaMigrationMarker = {
    version: 1,
    fromVersion: manifest.schemaVersion as 8 | 9 | 10 | 11 | 12,
...
export function migrateOpenedLibraryV8ToV9(input: {
  ...
  if (![8, 9, 10, 11, 12, 13].includes(input.manifest.schemaVersion) || input.manifest.schemaVersion >= SCHEMA_VERSION) {
