...
日期：2026-07-15  
状态：阶段 5A 已实现并部署；macOS 真机交互与阶段 4B 继续待验收
...
- 腾讯轻量云 HTTPS 部署已完成，`/health` 返回 `200 {"status":"ok"}`；Windows 两台物理设备已通过元数据、13 份原图、离线查看与恢复联网同步验收。
- GitHub macOS runner 已通过生产构建与 Electron 资料库链路；物理 Mac 安装、界面交互和跨平台实机同步仍待补验。用户已明确暂缓 Apple 签名与公证，因此它们不属于当前个人使用阶段的发布门槛。

## 当前验收证据

| 范围 | 当前证据 | 结论 |
| --- | --- | --- |
| 本地数据与离线能力 | Electron 资料库、SQLite、附件、备份恢复与 `journal.zip` 自动化 26/26 | 通过 |
| 同步协议 | 客户端 outbox/epoch/冲突测试、服务端鉴权/幂等/分页/附件哈希测试与双资料库 HTTP 集成测试 | 通过 |
| 线上服务 | 腾讯轻量云 HTTPS 健康检查返回 200 | 通过 |
| Windows 多设备 | 两台物理设备完成元数据、13 份原图、全部离线下载、断网打开与恢复同步 | 通过 |
| macOS 构建 | GitHub macOS runner 完成生产构建和 Electron 资料库 QA | 通过 |
| macOS 真机 | 未签名安装、界面操作、离线附件与 Windows↔macOS 实机同步 | 待补验 |
| 本地首屏性能 | 生产构建 Playwright 独立上下文 7 次采样：可交互 P50 70.1ms、P95 72.1ms；FCP P95 56ms；初始传输 287.2KiB / 6 请求；发布门禁持续校验 | 通过 |
| 同步时延 | 启动后 500ms 内发起同步、普通元数据同步 P95<5 秒仍缺少端到端定量采样 | 待量化 |
...
以下约束记录了方案选择时的原始状态；阶段 2～5A 已逐项补齐同步日志、附件哈希和远端恢复能力，保留本节用于说明为什么不直接同步 `journal.db`。
