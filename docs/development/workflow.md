...
发布脚本在精确提交的远端 Release Candidate 中执行一次 `qa:release`，不再先在本地重复整套检查；同一提交已经通过的候选认证可以复用。候选仍包含核心 UI、Electron 与存储恢复验证、发布演练及兼容性语料检查。

日常 CI 与候选认证执行 `benchmark:persistence --correctness-only`：10K/20K 各一次真实保存，校验数据、附件、并发冲突与退出保存；无效测量和正确性错误仍阻断。耗时超预算写入报告及日志告警，不以共享 runner 的单次耗时阻断发布。完整 QA 的定时或手动工作流继续执行扩展 UI 和严格性能门禁（5 次预热、30 次采样及原 SLO）。正式 tag 工作流仍必须通过双平台数据安全检查和最终安装包冒烟，全部资产齐全才公开 Release。
