...
test('常规 CI 在主干与拉取请求上运行同一质量门禁', () => {
  ...
  assert.match(workflow, /pnpm qa:release -- --skip-dashboard-10k/)
  assert.match(workflow, /performance:/)
  assert.match(workflow, /pnpm build/)
  assert.match(workflow, /node scripts\/qa-dashboard-10k\.mjs/)
  assert.match(workflow, /QA_PERFORMANCE_PROFILE:\s*hosted-windows/)
  assert.match(qualityGate, /process\.argv\.includes\('--skip-dashboard-10k'\)/)
  assert.match(qualityGate, /if \(!skipDashboard10k\)/)
})
