...
test('发布候选先认证精确 SHA，tag 流水线只做双平台安全构建', () => {
  ...
  assert.match(candidate, /pnpm qa:release/)
  assert.doesNotMatch(candidate, /pnpm qa:full/)
  assert.match(candidate, /pnpm benchmark:persistence --correctness-only\s*$/m)
...
test('本地发布在远端候选认证通过后才推送 tag，失败可沿用版本重试', () => {
  ...
  assert.doesNotMatch(release, /run\('pnpm', \['qa:release'\]\)/)
