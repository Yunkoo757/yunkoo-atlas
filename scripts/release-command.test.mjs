...
test('生产构建必须通过可重复的首屏性能门禁', () => {
  const pkg = JSON.parse(readFileSync('package.json', 'utf8'))
  const qualityGate = readFileSync('scripts/qa-release.mjs', 'utf8')

  assert.equal(pkg.scripts['qa:performance'], 'node scripts/qa-performance.mjs')
  assert.match(qualityGate, /\['qa:performance'\]/)
})

test('工作台 QA 限制整页重载并在关键冷启动前冲洗存储', () => {
  const qa = readFileSync('scripts/qa-workbench.mjs', 'utf8')
  const fullReloadCount = qa.match(/await gotoApp\(/g)?.length ?? 0
  const clientRouteCount = qa.match(/await navigateApp\(/g)?.length ?? 0

  assert.equal(fullReloadCount, 3, '只允许首次启动、持久化冷刷新与基准页启动整页加载')
  assert.ok(clientRouteCount >= 10, '页面覆盖应复用前端路由，避免 Windows Chromium 资源耗尽')
  assert.match(qa, /waitForPendingStorageOperations/)
  assert.match(qa, /flushPersistNow/)
})
...
test('发布流水线在 Windows 之后构建并上传 macOS 产物', () => {
  ...
  assert.match(workflow, /electron-builder --mac dmg zip --\$\{\{ matrix\.arch \}\} --publish never/)
  assert.match(workflow, /mac-\$\{\{ matrix\.arch \}\}\.dmg/)
  assert.match(workflow, /mac-\$\{\{ matrix\.arch \}\}\.zip/)
...
test('macOS 发布必须在打包前通过 Electron 数据链路验收', () => {
  ...
  const qaIndex = macJob.indexOf('pnpm qa:electron')
  const packageIndex = macJob.indexOf('electron-builder --mac dmg zip --${{ matrix.arch }} --publish never')
