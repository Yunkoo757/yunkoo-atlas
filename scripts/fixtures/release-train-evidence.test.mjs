...
test('QA 与性能最小伪造报告不能通过最终 manifest', () => {
  ...
  assert.equal(fullQaPassed({
    ...
    status: 'pass',
    mode: 'full',
    commands: ['qa:ci', 'qa', 'build:app', 'qa:dashboard-10k', 'qa:electron'],
  }), true)
...
test('最终质量清单必须由 publish 针对当前干净源码再次授权', () => {
  ...
  const manifest = {
    ...
    gates: {
      ...
      blobBridgeCoverage: { status: 'pass' },
    },
    checks,
...
test('Blob writer 发布必须有桥覆盖窗口与旧标签页确认', () => {
  const evidence = {
    version: 1,
    status: 'pass',
    ...
    coverageEndedAt: '2026-07-24T00:00:00.000Z',
    operator: 'Yunkoo',
    oldTabsRefreshedOrClosed: true,
  }
  assert.equal(blobBridgeCoveragePassed(evidence), true)
