...
test('E-FORCED-KILL 使用真实 Electron 主进程、原子临时文件观察和强杀后重开', () => {
  ...
  assert.match(runner, /gitTree: buildExpectation\.repository\.tree/)
  assert.match(runner, /sourceIdentity: buildExpectation\.repository\.sourceIdentity/)
  assert.match(runner, /runElectronMain\('identity'\)/)
  assert.match(runner, /validateBundleBuildIdentityEvidence\(bundleIdentity, \['main'\]\)/)
