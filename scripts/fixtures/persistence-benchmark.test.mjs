...
test('release 性能门冻结 10K/20K、5+30 采样、硬 SLO 与证据字段', () => {
  ...
  assert.match(releaseGate, /process\.env\.QA_PERFORMANCE_PROFILE \?\? 'local'/)
  assert.match(releaseGate, /benchmarkProfile === 'hosted-windows'/)
  assert.match(releaseGate, /relativeBaselineComparable/)
  assert.match(releaseGate, /relativeBaselineReason/)
