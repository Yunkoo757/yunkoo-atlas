...
export const THEME_COLOR_MIX_ALLOWLIST = Object.freeze([
  ...
  opticalMix('src/components/sidebar/SidebarWorkspace.css:545', '风险环内部图形按当前文字色生成低强度轨道。', '风险环改用独立 SVG 渐变 Token 时'),
  opticalMix('src/components/sidebar/SidebarWorkspace.css:558', '风险环内部图形按当前文字色生成中强度轨道。', '风险环改用独立 SVG 渐变 Token 时'),
  ...
  opticalMix('src/components/Sidebar.css:434', '风险环轨道需要在 App Surface 上进行低强度合成。', '风险环改用独立 SVG 渐变 Token 时'),
  opticalMix('src/components/Sidebar.css:445', '风险环进度需要继承状态文字色。', '风险环改用独立 SVG 渐变 Token 时'),
  opticalMix('src/components/Sidebar.css:489', '风险环 SVG 填充按 currentColor 生成。', '风险环改用独立 SVG 渐变 Token 时'),
  opticalMix('src/components/Sidebar.css:495', '风险环 SVG 描边按 currentColor 生成。', '风险环改用独立 SVG 渐变 Token 时'),
  opticalMix('src/components/Sidebar.css:504', '风险环 SVG 光晕按 currentColor 生成。', '风险环改用独立 SVG 渐变 Token 时'),
  ...
  opticalMix('src/components/trades/TradeList.css:343', '列表键盘定位同时混合 Focus 与行边界。', 'SelectionBox 能承载键盘定位时'),
  opticalMix('src/components/ui/Chip.css:38', '禁用 Chip 保留低强度底板以维持形状。', 'Chip 禁用态统一重构时'),
  ...
  opticalMix('src/views/ReviewSessionView.css:710', '空白图表区棋盘格用于表达可放置媒体区域。', '媒体占位区改为插图或纯色时'),
