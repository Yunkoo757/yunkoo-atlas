/* Adapted from DavidHDev/react-bits LiquidEther. See LIQUID-ETHER-LICENSE.txt.
 * React lifecycle replaced with an explicit mount/dispose function; fluid shaders retained.
 */
import * as THREE from './three.module.js';
export function mountLiquidEther(host, options={}) {
const {mouseForce=8,cursorSize=80,isViscous=true,viscous=60,iterationsViscous=32,iterationsPoisson=32,dt=.014,BFECC=true,resolution=.5,isBounce=false,colors=['#6366F1','#818CF8','#A78BFA'],autoDemo=true,autoSpeed=.3,autoIntensity=.6,takeoverDuration=.25,autoResumeDelay=5000,autoRampDuration=.6,backgroundColor='#101113',lightMode=false,visibilityGain=3}=options;
const mountRef={current:host},webglRef={current:null},resizeObserverRef={current:null},rafRef={current:null},intersectionObserverRef={current:null},isVisibleRef={current:true},resizeRafRef={current:null};
...
`;
    ...
    void main(){
    vec2 vel = texture2D(velocity, uv).xy;
    float lenv = clamp(1.0 - exp(-length(vel) * visibilityGain), 0.0, 1.0);
    vec3 c = texture2D(palette, vec2(lenv, 0.5)).rgb;
    float peak = max(c.r, max(c.g, c.b));
    vec3 chroma = clamp(c / max(peak, 0.0001), 0.0, 1.0);
    chroma = pow(chroma, vec3(1.25));
    vec3 ink = lightMode ? chroma : c;
    // Dark mode uses straight alpha: do not attenuate RGB a second time.
    vec3 outRGB = lightMode ? mix(bgColor.rgb, ink, lenv) : ink;
...
`;
        ...
        this.output = new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            uniforms: {
              ...
              visibilityGain: { value: visibilityGain }
            }
          })
        );
    ...
    // Position is provided by the Atlas hero background layer.
