...
async function run(): Promise<void> {
  ...
  try {
    ...
    assert(pendingWeek.getAttribute('aria-label')?.includes('待补做'), '有活动但未建档的周必须在可访问名称中标记待补做')
    assert(pendingWeek.getAttribute('data-review-week-state') === 'pending', '有活动但未建档的周必须标记 pending 状态')
    pendingWeek.click()
