  ...
  try {
    useStore.setState((state) => ({
      ...
      livePerformanceCycles: [
        { id: 'history', name: '历史', startTradingDayKey: '2026-01-01', createdAt: '2026-01-01T00:00:00.000Z' },
    ...
    const modeGroup = document.querySelector<HTMLElement>('[role="group"][aria-label="历史实盘内容"]')
    const filterShell = document.querySelector<HTMLElement>('.ui-filter-shell')
    assert(modeGroup && filterShell, '历史实盘缺少内容切换或筛选栏')
    assert(!filterShell.contains(modeGroup), '记录/案例切换不得混入筛选栏')
    ...
    caseButton.click()
    await waitFor(() => document.querySelector('[data-route]')?.textContent === '/live-history?view=cases', '关联案例没有更新路由身份')
