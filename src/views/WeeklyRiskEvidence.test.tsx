...
export function testWeeklyRiskEvidenceUsesDecisionFirstStructure(): void {
  ...
  assert(html.includes('<details class="wr-risk-daily"'), '每日风险轨迹必须使用可折叠的紧凑披露区')
  assert(!html.includes('<details class="wr-risk-daily" open=""'), '每日风险轨迹默认必须收起以减少版面占用')
  assert(html.includes('2 个交易日'), '每日风险摘要必须在收起时保留覆盖范围')
...
export function testWeeklyRiskDailyUsesNetBudgetAndExplainsIncompleteCoverage(): void {
  ...
  assert(!html.includes('该平仓日没有生效的风险规则'), 'complete 行不得展示 unknownReasons')
  assert(html.includes('原因：亏损交易缺少盈亏金额'), 'partial 行必须展示共享中文原因')
