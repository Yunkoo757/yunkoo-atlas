...
import type { Judgment, JudgmentSample, JudgmentTheme } from '@/lib/judgment/model'
...
export function JudgmentDeskView() {
  ...
  const [comparing, setComparing] = useState(false), [otherId, setOtherId] = useState('')
  ...
  useEffect(() => { setImageIndex(0); setResultId(null); setHistoryPage(0) }, [sample?.id, theme?.id])
  ...
  const move = (step: number) => {
    ...
    if (comparing) {
      ...
      if (candidates.length) setOtherId(candidates[(current + step + candidates.length) % candidates.length].id)
  ...
  const nextImage = (step: number) => { if (sample) setImageIndex(i => (i + step + sample.images.length) % sample.images.length) }
  ...
  return <div className="jd-view">
    ...
    <div className="jd-scroll"><div className="jd-rail">
      <div className="jd-heading"><div className="jd-topic"><Select ariaLabel="当前主题" value={theme?.id ?? ''} options={data.themes.map(t => ({ value: t.id, label: t.title }))} onValueChange={id => {
        setTraining(false); setComparing(false); update(d => ({ ...d, currentThemeId: id, currentSampleId: d.samples.find(s => s.themeId === id)?.id ?? null }))
      ...
      {!sample ? <div className="jd-empty"><h2>{theme ? '从一组有疑问的图开始' : '想弄清哪条规则的边界？'}</h2><p>{theme ? '收图上传或粘贴，也可在案例、日志图片上右键收录。' : '新建一个主题，再收录你想反复对照的图。'}</p></div> : <>
        <div className="jd-heading jd-sample-heading"><div className="jd-wrap jd-title-source"><h2 title={!blind ? sample.title : undefined}>{training ? '主题复盘' : sample.title || `素材 ${index + 1}`}</h2>{!blind && <SourceLink sourceTradeId={image?.sourceTradeId} />}</div>
          <div className="jd-row">{training ? <Button onClick={() => { setTraining(false); setResultId(null) }}>结束复盘</Button> : <><Button disabled={samples.length < 2} onClick={() => { setComparing(v => !v); setOtherId(samples.find(s => s.id !== sample.id)?.id ?? '') }}>{comparing ? '结束对照' : '对照'}</Button><Button title="随机复盘当前主题，每组素材一次" onClick={start}>随机复盘</Button></>}
        ...
        <div className={`jd-canvas${comparing ? ' jd-compare' : ''}`}>
          <section className="jd-panel">
            {comparing && <span className="jd-compare-label jd-muted">A · 固定当前素材</span>}
            ...
            <div className={`jd-image-nav${sample.images.length === 1 ? ' is-single' : ''}`} aria-hidden={sample.images.length === 1}><Button size="sm" disabled={sample.images.length === 1} aria-label="上一张图" onClick={() => nextImage(-1)}><ChevronLeft size={ICON_MD} /></Button><span>图 {imageIndex + 1} / {sample.images.length}</span><Button size="sm" disabled={sample.images.length === 1} aria-label="下一张图" onClick={() => nextImage(1)}><ChevronRight size={ICON_MD} /></Button></div>
          </section>
          {comparing && other && <CompareSample sample={other} candidates={samples.filter(s => s.id !== sample.id)} onSelect={setOtherId} />}
        </div>
        ...
        {!blind && sample.reference && <Button className="jd-reference" onClick={() => setModal('sample')}>参考：{JUDGMENT_LABELS[sample.reference.answer]}{sample.reference.needsReview ? ' · 待确认' : ''}</Button>}
      </>}
    </div></div>
...
function CompareSample({ sample, candidates, onSelect }: { sample: JudgmentSample; candidates: JudgmentSample[]; onSelect: (id: string) => void }) {
  const [index, setIndex] = useState(0)
  useEffect(() => setIndex(0), [sample.id])
  const image = sample.images[Math.min(index, sample.images.length - 1)]
  return <section className="jd-panel"><Select ariaLabel="对照素材 B" value={sample.id} options={candidates.map((s, i) => ({ value: s.id, label: `B · ${s.title || `素材 ${i + 1}`}` }))} onValueChange={onSelect} /><JudgmentImage key={image.assetId} assetId={image.assetId} group={sample.images.map(i => i.assetId)} sourceTradeId={image.sourceTradeId} /><div className="jd-image-nav"><Button size="sm" disabled={sample.images.length < 2} onClick={() => setIndex(i => (i - 1 + sample.images.length) % sample.images.length)}>上一张图</Button><span>{index + 1} / {sample.images.length}</span><Button size="sm" disabled={sample.images.length < 2} onClick={() => setIndex(i => (i + 1) % sample.images.length)}>下一张图</Button></div><span className="jd-muted">{sample.opinion ? JUDGMENT_LABELS[sample.opinion] : '还没有判断'}</span></section>
}
...
function ThemeEditor({ theme, onClose }: { theme?: JudgmentTheme; onClose: () => void }) {
  ...
  return <ModalShell title={theme ? '当前认识' : '新建主题'} onClose={onClose} footer={<><Button onClick={onClose}>取消</Button><Button variant="primary" onClick={() => {
...
function SampleEditor({ sample, onClose }: { sample: JudgmentSample; onClose: () => void }) {
  ...
  return <ModalShell title="备注与参考判断" onClose={onClose} footer={<><Button onClick={onClose}>取消</Button><Button variant="primary" onClick={() => {
