...
async function run(): Promise<void> {
  ...
  try {
    ...
    assert(presetLabels.some((label) => label.includes('亏损日志')), '开始页必须提供亏损日志复盘池')
    assert(presetLabels.some((label) => label.includes('错过机会')), '开始页必须提供错过机会复盘池')
    assert(presetLabels.some((label) => label.includes('近期多看')), '开始页必须提供近期多看复盘池')
