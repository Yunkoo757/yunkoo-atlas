// Generated from svg-1.svg; do not edit.
import { StaticLinearSvg } from '../StaticLinearSvg'
import type { LinearStaticIconProps } from '../types'

const body = "<path d=\"M8 1a4.25 4.25 0 0 1 4.25 4.25v.68c0 .83.302 1.632.849 2.258l.4.458c.323.369.502.843.502 1.333v1.637c0 .626-.509 1.134-1.135 1.134h-2.164a2.749 2.749 0 0 1-5.404 0H3.135A1.135 1.135 0 0 1 2 11.616V9.98c0-.49.179-.964.502-1.333l.4-.458a3.43 3.43 0 0 0 .849-2.258v-.68A4.25 4.25 0 0 1 8 1M6.855 12.75a1.249 1.249 0 0 0 2.29 0zM8 2.5a2.75 2.75 0 0 0-2.749 2.75v.68a4.93 4.93 0 0 1-1.22 3.246l-.4.457a.53.53 0 0 0-.13.346v1.271h9V9.98a.53.53 0 0 0-.13-.347l-.401-.457a4.93 4.93 0 0 1-1.22-3.246v-.68A2.75 2.75 0 0 0 8 2.5\"></path>"

export function LinearSubscribeIcon(props: LinearStaticIconProps) {
  return <StaticLinearSvg {...props} body={body} viewBox="0 0 16 16" />
}
