// Generated from svg-3.svg; do not edit.
import { StaticLinearSvg } from '../StaticLinearSvg'
import type { LinearStaticIconProps } from '../types'

const body = "<path d=\"M11.875 10.609a.3.3 0 0 1-.286.391H4.41a.3.3 0 0 1-.286-.391l.813-2.54A.1.1 0 0 1 5.033 8h5.934a.1.1 0 0 1 .095.07zM9.943 5a.1.1 0 0 0 .095-.13L8.867 1.209A.3.3 0 0 0 8.58 1H7.419a.3.3 0 0 0-.286.209l-1.171 3.66A.1.1 0 0 0 6.057 5zM4 12a1 1 0 0 0-1 1 2 2 0 0 0-1.99 1.8c-.011.11.08.2.19.2h13.6c.11 0 .2-.09.19-.2A2 2 0 0 0 13 13a1 1 0 0 0-1-1z\"></path>"

export function LinearConeIcon(props: LinearStaticIconProps) {
  return <StaticLinearSvg {...props} body={body} viewBox="0 0 16 16" />
}
