// Generated from svg-3.svg; do not edit.
import { StaticLinearSvg } from '../StaticLinearSvg'
import type { LinearStaticIconProps } from '../types'

const body = "<path fill-rule=\"evenodd\" d=\"M14.345 2.127a5.3 5.3 0 0 0-6.394-.052C6.068.948 3.677.934 1.79 2.095l-.327.202A.97.97 0 0 0 1 3.124V14.11a.849.849 0 0 0 1.34.693l.126-.09a4.18 4.18 0 0 1 4.962.095.94.94 0 0 0 1.144 0 4.18 4.18 0 0 1 4.962-.095l.127.09A.849.849 0 0 0 15 14.109V3.095a.94.94 0 0 0-.368-.747zM8.75 3.353a3.8 3.8 0 0 1 4.68-.037l.07.054v9.597a5.69 5.69 0 0 0-4.75-.008zm-1.5.051c-1.423-.887-3.25-.908-4.675-.03l-.075.045v9.548a5.69 5.69 0 0 1 4.75-.008z\" clip-rule=\"evenodd\"></path>"

export function LinearBookOpenIcon(props: LinearStaticIconProps) {
  return <StaticLinearSvg {...props} body={body} viewBox="0 0 16 16" />
}
