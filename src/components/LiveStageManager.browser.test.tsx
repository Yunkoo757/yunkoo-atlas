...
async function run(): Promise<void> {
  ...
  try {
    ...
    assert(text().includes('3实盘交易'), '弹窗必须展示当前阶段实盘交易数量')
    assert(text().includes('3实盘案例'), '弹窗必须展示当前阶段案例数量，案例状态不得混入实盘 blocker 计数')
    assert(text().includes('1周复盘'), '弹窗必须展示当前阶段周复盘数量')
    assert(text().includes('4风险记录'), '弹窗必须展示当前阶段风险记录总数')
    assert(text().includes('计划中 1 笔将保留在原阶段'), '弹窗必须中性说明计划记录归属')
    assert(text().includes('持仓中 1 笔'), '弹窗必须区分持仓中阻断项')
    assert(text().includes('周复盘可稍后补做'), '弹窗必须说明周复盘不再阻止切换')
    assert(text().includes('先处理持仓交易'), '弹窗必须区分持仓阻断与中性摘要')
    assert(text().includes('查看归档范围'), '弹窗必须提供按需查看归档范围的入口')
    assert(text().includes('策略、标签、模板和随记继续保留'), '弹窗必须解释保留范围')
